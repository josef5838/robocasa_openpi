...
from robocasa.utils.dataset_registry import DATASET_SOUP_REGISTRY, get_ds_meta
...
_CONFIGS = [
    ...
    TrainConfig(
        name="pi05-og_robocasa_pickplacetoastertocounter",
        model=pi0_config.Pi0Config(
            pi05=True,
            max_token_len=200,
        ),
        data=LeRobotRobocasaDataConfig(
            # Used as the normalization-stats asset key (written by scripts/compute_norm_stats.py)
            data_dirs=[get_ds_meta(task="PickPlaceToasterToCounter", split="pretrain", source="human")],
        ),
	    weight_loader=weight_loaders.CheckpointWeightLoader("gs://openpi-assets/checkpoints/pi05_base/params"),
        num_train_steps=20_000,
        batch_size=16,
